    ...
    let disposable = vscode.commands.registerCommand('assamesePython.run', async () => {
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                const pythonCode = await new Promise((resolve, reject) => {
                    // Use file argument instead of pipe
                    (0, child_process_1.exec)(`python "${transpilerPath}" "${tempInputPath}"`, { cwd: serverPath }, (error, stdout, stderr) => {
                        // Clean up temp file
                        try {
                ...
                const output = await new Promise((resolve, reject) => {
                    // Use file argument instead of pipe
                    (0, child_process_1.exec)(`python "${sandboxPath}" "${tempPythonPath}"`, { cwd: serverPath }, (error, stdout, stderr) => {
                        // Clean up temp file
                        try {
                            fs.unlinkSync(tempPythonPath);
                        }
                        catch (e) { }
                        if (error) {
